...
#    print(key, ':', value)
#print ("size = ", len(atomic))
#print("atomic = " , atomic);

#for i in flows:
#	print (i)

#print("size = " , len(flows))
...
#    print(key, ' : ', value)
#print("neighbor size = ",len(neighbor))
#exit()

#nkeys = sorted(list(neighbor.keys()))
#for i in nkeys:
#	print(i,neighbor[i])
...
def AV():
	...
	while patterns[size-1]:
		...
		for p in patterns[size-1]:
			...
			for src in p[0]:
				if src in neighbor:
					for n in neighbor[src]:
						...
						pprime = ((n,),p[1],p[2])
						if pprime in triples[1+len(p[1])+len(p[2])]:
							if n in atomic:
			...
			for dest in p[1]:
				if dest in neighbor:
					for n in neighbor[dest]:
						if n in p[1] or n in p[0]: continue
						expdest = tuple(sorted(list((p[1]+(n,)))))
	...
	# 	print('number of patterns:',len(patterns[s]))
	# 	print(patterns[s])
	# 	print('number of examined triples:',len(triples[s]))
	# 	print(triples[s])
...
# 	return set(neighbors)-set(region)-set(other)

# def get_neighbors(region, other): #slower
# 	neighbors = set()
# 	for l in region:
# 		if l in neighbor:
# 			neighbors.update(set(neighbor[l]))
# 	return neighbors-set(region)-set(other)
...
#	et = time.time()
#	print('Execution time of compute atomic + OD stats + prefix_sum:', et-st, 'seconds')
	...
	while patterns[size-1]:
		...
		# et = time.time()
		# print('Execution time at beginning of size=',size,':', et-st, 'seconds; numpatterns to extend=',len(patterns[size-1]))
...
#			if nextts <= 47:
	...
	# print('total measured time =',measuredtime)
	# print('wastedexp =',wastedexp1,wastedexp2,wastedexp3)
	return patterns
